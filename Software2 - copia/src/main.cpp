...
void loop() {
  ...
  if (tNow-tLast > dT_MICROSECONDS) {
    ...
    if (enableControl) {
      ...
      if (/*IBus.isActive()*/ true) { // Check if receiver is active
        speedInput = ((float) /*IBus.readChannel(1)*/1500-1500)/5.0; // Normalise between -100 and 100
        steerInput = ((float) /*IBus.readChannel(0)*/1500-1500)/5.0;
      }
